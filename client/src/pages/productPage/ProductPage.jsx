...
const ProductPage = () => {
    ...
    const fetchProducts = async () => {
      try {
        const response = await axios.get(
          ` https://trendify-bese27server.vercel.app/api/products/filter?category=${category}`
  ...
  const handleCategory = async (cat) => {
    ...
    try {
      ...
      const response = await axios.get(
        `  https://trendify-bese27server.vercel.app/api/products/filter?category=${cat}`
      );
      setProducts(response.data);
  ...
  const handleViewAll = async () => {
    try {
      const response = await axios.get(`https://trendify-bese27server.vercel.app/api/products/`); // Change the limit as needed
